...
| `/upload_refer` | POST | 上传参考音频文件 |
| `/list_refers` | GET | 列出已上传的参考音频 |
| `/download_refer/{name}` | GET | 下载已上传的参考音频 |
| `/delete_refer/{name}` | DELETE | 删除已上传的参考音频 |
...
API 内置参考音频管理功能，支持从外部服务上传参考音频进行语音克隆：

```bash
# 上传参考音频
curl -F "file=@voice.wav" http://localhost:9880/upload_refer

# 指定文件名上传
curl -F "file=@voice.wav" "http://localhost:9880/upload_refer?name=linxia.wav"

# 列出已上传的参考音频
curl http://localhost:9880/list_refers

# 下载参考音频
curl http://localhost:9880/download_refer/linxia.wav -o linxia.wav

# 删除参考音频
curl -X DELETE http://localhost:9880/delete_refer/linxia.wav
```

上传目录通过环境变量 `GPT_SOVITS_UPLOAD_DIR` 指定，默认 `./refs`。
上传后可直接在 TTS 推理中使用该路径：

```bash
# 使用刚上传的音频作为参考音频进行语音克隆
curl -X POST "http://localhost:9880/" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "你好，这是我的克隆声音",
    "text_language": "zh",
    "refer_wav_path": "refs/linxia.wav",
